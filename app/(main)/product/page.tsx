...
export default function ProductPage() {
  ...
  return (
    <div className="bg-background text-on-background min-h-screen">
      {/* Main Content Canvas */}
      <main className="max-w-7xl mx-auto px-margin-mobile space-y-6 pt-[80px]">
        ...
        <div className="flex overflow-x-auto gap-sm pb-sm hide-scrollbar -mx-margin-mobile px-margin-mobile">
          {CATEGORIES.map((category, index) => (
            <button 
              ...
              className={`px-md py-sm rounded-full font-label-lg whitespace-nowrap active:scale-95 transition-transform ${index === selectedCategory ? "bg-primary-container text-on-primary-container" : "bg-surface-container-high text-on-surface-variant"}`}
            >
              {category}
            </button>
          ))}
        ...
        <div className="grid grid-cols-2 gap-gutter">
          {PRODUCTS.map((product) => (
            <Link key={product.id} href={`/product/${product.id}`} className="bg-surface-container-lowest rounded-xl overflow-hidden shadow-[0px_4px_12px_rgba(0,0,0,0.08)] flex flex-col cursor-pointer group">
